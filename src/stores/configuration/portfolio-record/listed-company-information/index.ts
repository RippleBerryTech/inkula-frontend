...
export const useListedCompanyInformationStore = defineStore('listed-company-information', {
  state: () => ({
  ...
  actions: {
    async fetchListedCompanyInformation(force = false) {
      if (this.hasFetchedPortfolioRecords && !force) return
      this.loading = true;
      this.error = '';

      try {
        const res = await api.get('/configuration/portfolio-record/listed-company-information/list'); // assuming your endpoint is /roles
    ...
    async addListedCompanyInformation(data) {
      this.addPortfolioRecordError = '';
      this.addPortfolioRecordFieldErrors = {};
      this.loading = true;

      try {
        const res = await api.post('/configuration/portfolio-record/listed-company-information/store', data);
        if (res.data.success) {
          this.portfolioRecords.push(res.data.data.listedCompanyInformation);
          return true;
        } else {
    ...
    //edit sector
    async editListedCompanyInformation(id) {
      this.editPortfolioRecordError = '';
      this.loading = true;
      try {
        const res = await api.get(`/configuration/portfolio-record/listed-company-information/edit/${id}`);
        if (res.data.success) {
    ...
    async updateListedCompanyInformation(data) {
      this.editPortfolioRecordError = '';
      this.editPortfolioRecordFieldErrors = {};
      this.loading = true;

      try {
        const res = await api.put(`/configuration/portfolio-record/listed-company-information/update/${data.id}`, data);
        if (res.data.success) {
          this.portfolioRecords = this.portfolioRecords.map((record) => {
            if (record.id === data.id) {
              return res.data.data.listedCompanyInformation;
            }
            return record;
          });
          return true;
    ...
    async deleteListedCompanyInformation(id) {
      this.deletePortfolioRecordError = '';
      this.loading = true;
      try {
        const res = await api.delete(`/configuration/portfolio-record/listed-company-information/delete/${id}`);
        if (res.data.success) {
    ...
    async downloadTemplate() {
      ...
      this.error = '';
      try {
        const response = await api.get('/configuration/portfolio-record/listed-company-information/download-import-template', {
        ...
        link.href = url;
        link.setAttribute('download', 'listed-company-information_template.xlsx'); // file name
