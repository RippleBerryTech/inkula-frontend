    ...
    <div>
        ...
        <div class="panel pb-0 mt-6">
            <div class="flex md:items-center md:flex-row flex-col mb-5 gap-5">
                ...
                <router-link to="/economic-management/sectors/add" v-if="hasPermission('Add Sector')" class="btn btn-primary">Add Sector</router-link>
            </div>
                ...
                <vue3-datatable :rows="sectorStore.sectors" :columns="columns" :totalRows="sectorStore.sectors?.length"
                    ...
                    <template #action="data">
                        <div class="flex items-center">
                            <div>
                                <router-link v-if="hasPermission('Edit Sector')" :to="{ name: 'sector-edit', params: { id: data.value.id } }"
                            ...
                            <div>
                                <button v-if="hasPermission('Delete Sector')" type="button" v-tippy:delete>
                                    <IconTrash :size="20" stroke-width="1.5"  @click="deleteSectorModal = true; selectSectorId = data.value.id"/>
                                </button>
                            </div>
                        </div>
                    </template>
