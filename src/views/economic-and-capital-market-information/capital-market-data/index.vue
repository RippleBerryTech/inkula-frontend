                ...
                <vue3-datatable v-else :rows="capitalMarketDataStore.economicSubmissions" :columns="columns" :totalRows="capitalMarketDataStore.economicSubmissions?.length"
                    ...
                    <template #action="data">
                        <div class="flex items-center">
                            <div>
                                <router-link v-if="hasPermission('View Economic Submission')" :to="{ name: 'capital-market-data-show', params: { id: data.value.id } }"
                                    class="ltr:mr-2 rtl:ml-2 group flex items-center" v-tippy="'View Details'">
                                    <IconEye :size="20" stroke-width="1.5" />
                                </router-link>
                            </div>
                            <div>
                                <router-link v-if="hasPermission('Edit Economic Submission')" :to="{ name: 'capital-market-data-edit', params: { id: data.value.id } }"
...
useMeta({ title: 'Capital Market Data' });
const search = ref('');
const columns =
    ref([
        { field: 'id', title: '#' },
