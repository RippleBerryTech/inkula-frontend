                    ...
                    <div class="w-full max-w-[440px] lg:mt-16">
                        ...
                        <form class="space-y-5 dark:text-white" @submit.prevent="submit">
                            ...
                            <button
                                ...
                                <span>{{ auth.loading ? 'Signing up...' : 'Sign Up' }}</span>
                            </button>
...
import { IconLock, IconMail, IconUser } from '@tabler/icons-vue';
...
const submit = async () => {
  ...
  await $v.value.$validate()
    ...
    if (!$v.value.$invalid) {
        var res = await auth.register(form)
        if (res) {
            //reset form
            form.email = ''
            form.password = ''
